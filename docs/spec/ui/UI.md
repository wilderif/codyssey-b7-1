...
- Chat 항목은 사용자 질문을 오른쪽에, AI 답변과 실패 안내를 왼쪽에 배치해 발화 주체를
  구분합니다.

### 상태 안내

- AI 답변 위치의 Loading 영역에 `aria-live="polite"`를 적용합니다.
- 즉시 확인해야 하는 form 오류와 실패 Chat 항목은 `role="alert"`로 알립니다.
- Chat 성공은 별도 status 문구 대신 pending Chat 항목의 Loading을 실제 답변으로 교체해
  표시합니다.
...
- 질문 `textarea`, 전송 button, request 상태 영역을 제공합니다.
- 질문 control에는 `required`와 `maxlength="1000"`을 적용합니다.
- `maxlength="1000"`은 일반적인 입력 과정에서 1000자 초과 작성을 제한합니다. JavaScript의
  길이 검증은 programmatic value 변경처럼 HTML constraint가 우회된 상황을 위한 방어입니다.
...
- 실패 Chat 항목은 현재 Browser 화면의 처리 결과이며 영구 저장을 보장하지 않습니다. 새로고침
  후에는 `GET /chat`이 rendering한 server history를 기준으로 다시 표시합니다.
- 성공·실패와 관계없이 현재 작성 중인 다음 질문 draft를 비우거나 전송한 질문을 복원하지
  않습니다.
- Redirect를 제외한 공통 종료 경로는 Loading 안내를 정리하고 전송 button을 다시 활성화한 뒤
...
Request를 시작한 뒤 받은 Chat API 오류는 다음 기준으로 pending 항목을 처리합니다.
...
- [ ] 이전 대화가 본인 record만 과거부터 최신 순서로 표시되고 최신 대화가 최하단에 위치함
- [ ] 빈 history와 실패 record가 계약된 안내 문구로 표시됨
- [ ] 빈 문자열·공백·1자·1000자·1000자 초과 입력을 검증함
- [ ] 유효한 전송 직후 질문이 입력창에서 제거되고 Chat 항목의 오른쪽에 즉시 표시됨
- [ ] 전송한 질문의 왼쪽 AI 답변 위치에 `답변 생성 중…`이 표시되고 빈 history 안내가 제거됨
- [ ] Request 중 button이 비활성화되고 중복 전송이 차단되지만 다음 draft는 작성할 수 있음
- [ ] 성공 후 같은 항목의 Loading이 답변으로 교체되고 중복 항목 없이 새 draft가 유지됨
- [ ] 실패 후 같은 항목의 Loading이 안전한 `detail`로 교체되고 새 draft가 유지됨
- [ ] 실패 Chat 항목에 임의 ID·시각이 없고 새로고침 후 server history로 대체됨
- [ ] Network·비정상 응답에서 기본 오류 문구를 포함한 실패 Chat 항목과 상태 복구가 동작함
