...
| `app/chat` | 질문 검증, 최근 성공 대화 5개 문맥, OpenAI 호출, ChatExchange 저장·조회와 운영 metadata |
| `app/core` | environment variable, SQLAlchemy Base·DB session, 보안, 공통 logging |
| `app/ui` | Jinja2 화면 router, chat·본인 기록·관리자 운영 metadata template, CSS·JavaScript static asset |
...
| `app/chat/**` | 질문 검증, 문맥 구성, OpenAI 호출, ChatExchange CRUD·저장·조회 | 이상헌 |
...
- `app/ui`는 Auth·Chat Service만 호출하고 Repository와 OpenAI를 직접 호출하지 않습니다.
...
def list_admin_chat_operation_metadata(*, db: Session) -> list[AdminChatOperationMetadataItem]: ...
def list_chat_exchange_history(*, user_id: int, db: Session) -> list[ChatExchangeHistoryItem]: ...
...
- `/admin/logs`는 `chat_exchanges`의 사용자별 운영 metadata를 읽기 전용으로 표시합니다.
  UI router는 `require_admin`으로 접근을 검사한 뒤
  `list_admin_chat_operation_metadata()`만 호출합니다. 질문·답변 원문과 내부 `error_message`는
  기본 표시에서 제외합니다.
- `app/main.py`는 UI·Chat router, SessionMiddleware, logging, health, `init_db()`를 연결하고,
